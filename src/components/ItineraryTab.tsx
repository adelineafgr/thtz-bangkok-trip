...
export const ItineraryTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Single Day Detail / Timeline View */}
      {viewMode === 'timeline' && (
        <>
          {/* Day Selector Bar */}
            ...
            {itinerary.map(day => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-[140px] sm:min-w-[160px] p-3.5 rounded-2xl border text-left transition shrink-0 ${
                    isActive
                      ? 'bg-indigo-600 text-white border-indigo-600 shadow-md ring-2 ring-rose-400'
                      : 'bg-white text-indigo-950 border-indigo-100 hover:border-indigo-300 hover:bg-indigo-50/40'
                  }`}
                >
                  <div className="flex items-center justify-between text-[11px] font-black mb-1">
                    <span className={isActive ? 'text-amber-300' : 'text-rose-500 uppercase tracking-wider'}>
                      Day {day.dayNumber}
                    </span>
                    <span className={`text-[10px] font-bold ${isActive ? 'text-indigo-200' : 'text-indigo-400'}`}>
                      {day.date}
                    </span>
                  </div>
          ...
          {/* Active Day Banner */}
          {activeDay && (
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-4 sm:p-5 border border-amber-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs font-medium text-slate-600 mt-0.5">
                  ✨ {activeDay.themeSubtitle}
                </p>
              </div>

              <div className="flex items-center space-x-2">
                <button
                  onClick={() => openAddModal(activeDay.dayNumber)}
                  className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-xs flex items-center space-x-1 transition"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>Add Activity</span>
                </button>
                <div className="text-xs text-slate-500 bg-white/80 px-3 py-1.5 rounded-xl border border-amber-100 flex items-center space-x-1.5">
                  <Clock className="w-3.5 h-3.5 text-amber-600" />
                  <span>{activeDay.activities.length} Activities</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3 relative before:absolute before:inset-0 before:left-5 sm:before:left-6 before:w-0.5 before:bg-slate-200 before:z-0">
            {activeDay.activities.length === 0 ? (
              <div className="bg-white rounded-2xl p-8 text-center text-slate-400 text-xs border border-dashed border-slate-300">
                No activities scheduled for this day yet. Click "Add Activity" to plan your day!
              </div>
            ) : (
              activeDay.activities.map((act) => (
                <div
                  key={act.id}
                  onClick={() => openDetailModal(selectedDayNum, act)}
                  className={`relative z-10 bg-white rounded-2xl p-4 sm:p-5 border transition shadow-2xs cursor-pointer ${
                    act.isDone ? 'bg-slate-50/80 border-slate-200 opacity-75' : 'border-slate-200 hover:border-amber-300'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start space-x-3 sm:space-x-4">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleItineraryActivityDone(selectedDayNum, act.id);
                        }}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <span className="px-2.5 py-0.5 bg-slate-900 text-amber-400 rounded-md text-xs font-mono font-bold">
                            {act.time}
                          </span>
                          <span
                            className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-md text-[11px] font-semibold border ${getCategoryBg(
                              act.category
                            )}`}
                          >
                          ...
                          {act.estimatedBudgetTHB && (
                            <span className="text-[11px] font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">
                              Est: {formatCurrency(act.estimatedBudgetTHB)}
                            </span>
                          )}
                        </div>

                        {/* Title & Description */}
                        <h4 className={`text-base font-bold text-slate-900 ${act.isDone ? 'line-through text-slate-500' : ''}`}>
                          {act.title}
                        </h4>
                        ...
                        {/* Location Name & Google Maps button */}
                        {(act.locationName || act.mapsUrl) && (
                          <div className="flex items-center space-x-2 mt-2 pt-2 border-t border-slate-100 text-xs">
                            <MapPin className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                            <span className="font-medium text-slate-700 truncate">
                              {act.locationName || 'Google Maps Location'}
                            </span>
                            {act.mapsUrl && (
                              <a
                                href={act.mapsUrl}
                                ...
                                rel="noopener noreferrer"
                                onClick={(e) => e.stopPropagation()}
                                className="inline-flex items-center space-x-1 px-2 py-0.5 bg-amber-100 text-amber-800 hover:bg-amber-200 rounded-md text-[10px] font-bold transition"
                              >
                                <span>Open Maps</span>
                    ...
                    <div className="flex items-center space-x-1 shrink-0">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          openEditModal(selectedDayNum, act);
                        }}
                        className="p-1 text-slate-400 hover:text-indigo-900 hover:bg-indigo-50 rounded-lg transition"
                        title="Edit activity"
                      >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleDeleteActivity(selectedDayNum, act.id, act.title);
                        }}
